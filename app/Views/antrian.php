        ...
        <div class="card">
            <div class="card-body">
                ...
                <p class="card-description">
                    Pembeli yang memiliki pesanan belum siap.
                </p>
    ...
    <div class="col-lg-6 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <p class="card-description">
                    Pesanan pembeli yang sudah siap hari ini.
                </p>
                <div class="table-responsive">
...
<script>
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.length) {
                let no = 1; // Initialize the counter
                for (let i = 0; i < data.length; i++) {
                    isiPesanan += "<tr><td>" + no++ + "</td><td>" + data[i].nama + "</td><td>";

                    if (data[i].status == 0) {
                        isiPesanan += "<label class='badge badge-danger'>Bayar";
                    } else {
                        isiPesanan += "<label class='badge badge-success'>Proses";
                    }

                    isiPesanan += "</label></td><td><button href='#' class='btn btn-inverse-warning btn-sm' onClick='modalRincian(" + data[i].id + ", \"" + data[i].nama + "\", " + no + "," + data[i].status + ")'><i class='mdi mdi-format-list-bulleted-type'></i><i class='mdi mdi-food-fork-drink'></i></button></td></tr>";
                }
            } else {
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.length) {
                let no = 1; // Initialize the counter
                for (let i = 0; i < data.length; i++) {
                    isiPesanan += "<tr><td>" + no++ + "</td><td>" + data[i].nama + "</td><td><label class='badge badge-success'>Selesai</label></td><td><button href='#' class='btn btn-inverse-success btn-sm' onClick='modalRincian(" + data[i].id + ", \"" + data[i].nama + "\", " + no + "," + data[i].status + ")'><i class='mdi mdi-playlist-check'></i><i class='mdi mdi-food'></i></button></td></tr>";
                }
            } else {
